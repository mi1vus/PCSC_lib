...
// 	ATrans: = @FTrans;
	 /*TSDIAppForm(Ctx).*/
	char buff[BuffSize];
	snprintf(buff, sizeof(buff), "Получение ниформации о заказе, TransactID: %ll", TransactID);
	AddText((char*)buff);
...
__int64 SetDDelegate(__int64 Pump, __int64 Osnovan, void* Ctx)
{
	...
	char buff[BuffSize];
	snprintf(buff, sizeof(buff), "Установка дозы на ТРК: %i , сгенерирован TransID = %ll", Pump, TransCounter);
	AddText((char*)buff);
...
TDispStatusInfo* GetDDelegate(__int64 Pump, void* ctx)
{
	...
	_Res = true;
	TDispStatusInfo *CRD_DISPSTATUS;
	try
	{
	...
	void* ctx)
{
	...
	Trans_ID, Price / 100, Amount / 100, DiscountMoney / 100);
...
//			std::thread t1(&DebitThread::Execute);
			...
			if (FSmartPumpControlLink->Open != nullptr)
			{
				if (FSmartPumpControlLink->Open(
					...
					&SaveRcpt_Delegate, "Sample Control", nullptr) != 1)
				{
					std::cout << "Ошибка подключения библиотеки";
					std::cin >> menu;
					return 0;
			...
			*/
			...
			if (FSmartPumpControlLink->FuelPrices != nullptr)
			...
			*/
			...
			if (FSmartPumpControlLink->PumpFuels != nullptr)
