...
public:
	...
	SmartPumpControl(char* AFilePath, /*PSmartPumpControlLink* ALink, */ char* ADescription)
	{
		...
		if (!IsStatusOK)
		{
			if (ADescription != 0)
			{
				strcat_s(ADescription,strlen(ADescription), AFilePath);
				strcat_s(ADescription, strlen(ADescription), " библиотека не найдена!");
			}
			return;
		}
		...
		if (!IsStatusOK)
		{
			...
			{
				strcat_s(ADescription, strlen(ADescription), AFilePath);
				strcat_s(ADescription, strlen(ADescription), " не получается открыть библиотеку!");
			}
			return;
		}
